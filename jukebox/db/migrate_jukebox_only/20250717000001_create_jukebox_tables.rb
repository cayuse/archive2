class CreateJukeboxTables < ActiveRecord::Migration[8.0]
  def change
    ...
    create_table :jukebox_playlists do |t|
      ...
      t.uuid :archive_playlist_id, null: false  # References archive playlists (UUID)
    ...
    create_table :jukebox_queue_items do |t|
      t.uuid :song_id, null: false  # References archive songs (UUID)
      t.uuid :user_id  # Optional - who requested it (UUID)
      t.integer :position, null: false
    ...
    add_foreign_key :jukebox_queue_items, :users, on_delete: :nullify, type: :uuid
    add_foreign_key :jukebox_cached_songs, :songs, on_delete: :cascade, type: :uuid
  end
end
