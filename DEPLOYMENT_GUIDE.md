...
Prepare two env files on each node in `archive/` and always source them in this order.

1) `env.common` (shared defaults)
```bash
export POSTGRES_PASSWORD=...              # required
export POSTGRES_DATA_PATH=/abs/path/pg    # required
export HOST_STORAGE_PATH=/abs/path/storage
...
# Bucardo local (inside docker network)
...
2) `env.master` (on the master only)
```bash
export ARCHIVE_ROLE=master
export APP_HOST=<master_ip_or_dns>
# No Bucardo container on master in this phase
```

3) `env.slave` (on each slave)
```bash
export ARCHIVE_ROLE=slave
export APP_HOST=<slave_ip_or_dns>

# Master DB connection (reachable via VPN)
export MASTER_DB_HOST=<master_vpn_ip>
...
# Bucardo will replicate FROM master TO this machine
