class SongsController < ApplicationController
  ...
  def create
    ...
    @song.processing_status = 'new' if @song.processing_status.blank?
  ...
  def edit
    # If song is new and has audio file, extract metadata
    # Check for new_import? or nil/blank processing_status
    if (@song.new_import? || @song.processing_status.blank?) && @song.audio_file.attached?
      ...
      flash[:notice] = "Metadata was extracted and applied. Please review and save."
    else
  ...
  def maintenance
    ...
    @status_filter = params[:status] || 'needs_attention'
    @songs = policy_scope(Song)
                  .includes(:artist, :album, :genre)
                  .order(created_at: :desc)
